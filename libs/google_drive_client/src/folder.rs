use super::{
    ...
    responses::{FilesUploadResponse, FilesUploadResponseOk},
};
...
impl GoogleDriveFolder {
    ...
    async fn find_sub_folder_id_for_name(
        &self,
        subfolder_name: &str,
        ...
        let query = format!(
            "(mimeType = 'application/vnd.google-apps.folder') and \
                             ('{}' in parents) and \
                             (name = '{}')",
            self.info.id, subfolder_name
        );

        let mut files_list = get_files_list_with_query(&self.request_builder, &query, None).await?;

        while !files_list.files.is_empty() {
            // Обходим найденный список
            // TODO: Можно было бы убрать фильтрацию тут
            let found_res = files_list.files.into_iter().find(|val| {
                ...
                val.mime_type.eq("application/vnd.google-apps.folder")
                    ...
                    && parents
                        .iter()
                        .any(|id| id.as_str().eq(self.info.id.as_str()))
            });
            ...
            if files_list.next_page_token.is_some() {
                files_list = get_files_list_with_query(
                    &self.request_builder,
                    &query,
                    files_list.next_page_token,
                )
                .await?;
            } else {
                break;
    ...
    async fn create_sub_folder(
        &self,
        subfolder_name: &str,
    ) -> Result<GoogleDriveFolder, GoogleDriveError> {
        ...
        Ok(GoogleDriveFolder::new(
            self.request_builder.clone(),
            response,
        ))
    }

    pub async fn create_subfolder_if_needed(
        &self,
        subfolder_name: &str,
    ) -> Result<GoogleDriveFolder, GoogleDriveError> {
        let found_sub_folder_id = self.find_sub_folder_id_for_name(&*subfolder_name).await?;
